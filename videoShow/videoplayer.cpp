...
static void SetVolume(uint8_t* buf, int32_t size, int32_t uRepeat, double vol);
...
int audio_decode_frame(VideoState *is, uint8_t *audio_buf, int buf_size)
{
    ...
    for(;;)
    {
        ...
        while(audio_pkt_size > 0)
        {
          ...
          //  SetVolume(audio_buf, data_size, 1, is->m_player->m_Volumn);
...
void VideoPlayer::SetVolume(int nValue)
{
//    m_Volumn = nValue*1.00/100;
    //声音是一种对数关系, 使用对数会增加计算时间, 这里使用反比例函数代替  y= 1/(101-x);
    // 变化 y= 4/(104-x); 100 -> 100% 99 --> 80%  98 ->67%   2   3
    if( nValue == 0 )
    {
        m_Volumn = 0;
    }else
    {
        m_Volumn = 4.00/(104 - nValue);
    }
